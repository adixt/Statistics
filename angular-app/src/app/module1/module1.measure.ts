...
export class AverageMeasures {
  ...
  static confidence(m, n, p, k) {
    ...
    let r = m / n;
    let resultLower = r - z * Math.sqrt((r * (1 - r)) / n);
    let resultHigher = r + z * Math.sqrt((r * (1 - r)) / n);

    console.log('resultLower:' + resultLower);
    console.log('resultHigher:' + resultHigher);

    let result = "od " + (resultLower * k) + " do " + (resultHigher * k);
    return result;
  }
  static confidenceZnaneOdchylenie(Odchylenie, wielkosc, srednia, z) {
    console.log("LuuL :" + z);
    ...
    let r = z * ((Odchylenie) / (Math.sqrt(wielkosc)));

    let resultLower = srednia - r;
    let resultHigher = srednia + r;

    console.log('Margines błedu  :' + r);

    console.log('resultLower ZnaneOdchylenie :' + resultLower);
    console.log('resultHigher ZnaneOdchylenie:' + resultHigher);

    let result = "od " + (resultLower) + " do " + (resultHigher)+ " margines błędu : "+r;
    return result;
  }
}